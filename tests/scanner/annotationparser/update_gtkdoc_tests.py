...
if __name__ == '__main__':
    gi_tests = os.path.abspath(os.path.join(os.path.dirname(__file__), 'gtkdoc'))
    gtkdoc_tests = os.path.abspath(r'/home/dieterv/gnome.org/checkout/gtk-doc/tests')
    ...
    for root, dirs, files in os.walk(gtkdoc_tests):
        for inputfile in files:
            ...
            if 'src' in root and ext in ['.c', '.h']:
                ...
                outputfile = os.path.join(gi_tests, "%s.in" % relpath)
                with io.open(outputfile, 'w', newline='') as f:
                    f.write(HEADER)
                    for chunk in chunks:
                        docblock = parser.parse_comment_block(chunk[0].rstrip(), chunk[1], chunk[2])
                        if docblock:
                            f.write(TEST % {'docblock': chunk[0].rstrip(), 'tree': parsed2tree(docblock), 'serialized': writer.write(docblock).rstrip()})
                    f.write(FOOTER)
